...
export const sendPhoneVerificationCodeFirebase = async (phoneNumber, appVerifier) => {
  try {
...
export const confirmPhoneNumberFirebase = async (confirmationResult, verificationCode) => {
  try {
    const userCredential = await confirmationResult.confirm(verificationCode);
    const user = userCredential.user;

    // Send user data to backend to get JWT token or create user
    const response = await fetch("/api/auth/firebase-phone", {
      ...
      body: JSON.stringify({
        ...
        displayName: user.displayName, // Will be null for phone auth unless set separately
        // photoURL: user.photoURL, // Will be null for phone auth unless set separately
      }),
    });
