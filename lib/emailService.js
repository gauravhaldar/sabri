...
export async function sendOrderNotificationEmail(orderData) {
  try {
    ...
    const itemsList = items.map((item, index) => `
      <tr>
        <td style="padding: 15px; border: 1px solid #e0e0e0; vertical-align: middle;">
          ...
          <div style="font-size: 12px; color: #666;">SKU: ${item.sku || item.id || 'N/A'}</div>
        </td>
        <td style="padding: 15px; border: 1px solid #e0e0e0; text-align: center; vertical-align: middle;">
          <span style="background-color: #f8f9fa; padding: 4px 8px; border-radius: 4px; font-weight: bold;">${item.quantity}</span>
        </td>
        <td style="padding: 15px; border: 1px solid #e0e0e0; text-align: right; vertical-align: middle;">
          <span style="font-weight: bold; color: #333;">₹${item.price?.toLocaleString() || 'N/A'}</span>
        </td>
        <td style="padding: 15px; border: 1px solid #e0e0e0; text-align: right; vertical-align: middle;">
          <span style="font-weight: bold; color: #007bff; font-size: 16px;">₹${((item.price || 0) * (item.quantity || 1)).toLocaleString()}</span>
        </td>
      </tr>
    `).join('');
          ...
          <div class="order-info">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                ...
                <td style="padding: 12px;">${new Date(createdAt).toLocaleString('en-IN', { 
              ...
              <tr>
                ...
                <td style="padding: 12px;">${paymentMethod === 'online_payment' ? '💳 Online Payment' : '💰 Cash on Delivery'}</td>
              </tr>
            </table>
          </div>
          ...
          <div class="summary">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                ...
                <td style="padding: 8px; text-align: right;">₹${orderSummary.subtotal?.toLocaleString() || '0'}</td>
              </tr>
              <tr>
                <td style="padding: 8px; font-weight: bold;">Shipping Charges:</td>
                <td style="padding: 8px; text-align: right;">₹${orderSummary.shippingCharges?.toLocaleString() || '0'}</td>
              </tr>
              <tr>
                <td style="padding: 8px; font-weight: bold;">Coupon Applied:</td>
                <td style="padding: 8px; text-align: right;">${orderSummary.couponCode || 'None'}</td>
              </tr>
              <tr>
                <td style="padding: 8px; font-weight: bold;">Discount:</td>
                <td style="padding: 8px; text-align: right;">-₹${orderSummary.discount?.toLocaleString() || '0'}</td>
              </tr>
              <tr style="border-top: 2px solid #333;">
                <td style="padding: 8px; font-weight: bold; font-size: 16px;">Total Amount:</td>
                <td style="padding: 8px; text-align: right; font-weight: bold; font-size: 16px;">₹${orderSummary.total?.toLocaleString() || '0'}</td>
              </tr>
            </table>
          </div>
    ...
    const textContent = `
      ...
      - Status: ${status}
      - Payment: ${paymentMethod === 'online_payment' ? 'Online Payment' : 'Cash on Delivery'}
      ...
      Order Items:
      ${items.map(item => `- ${item.name} (Qty: ${item.quantity}) - ₹${item.price?.toLocaleString()}`).join('\n')}
      
      Order Summary:
      - Subtotal: ₹${(orderSummary.subtotal || 0).toLocaleString()}
      - Shipping: ₹${(orderSummary.shippingCharges || 0).toLocaleString()}
