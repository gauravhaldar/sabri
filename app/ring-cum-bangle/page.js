...
import { useFineGold } from "../../hooks/useProducts";
...
export default function FineGoldPage() {
  ...
  useEffect(() => {
    const savedPosition = sessionStorage.getItem('ring-cum-bangle-scroll-position');
    const savedProductId = sessionStorage.getItem('ring-cum-bangle-last-product');
    const savedVisibleCount = sessionStorage.getItem('ring-cum-bangle-visible-count');
    const savedProducts = sessionStorage.getItem('ring-cum-bangle-loaded-products');
    ...
    if (savedProducts) {
      sessionStorage.removeItem('ring-cum-bangle-loaded-products');
    }
  }, []);
        ...
        if (productIndex !== -1) {
          ...
          if (itemsNeeded > visibleCount) {
            ...
            setTimeout(() => {
              ...
              // Clear saved state
              sessionStorage.removeItem('ring-cum-bangle-last-product');
              sessionStorage.removeItem('ring-cum-bangle-scroll-position');
              sessionStorage.removeItem('ring-cum-bangle-visible-count');
  ...
  useEffect(() => {
    ...
    const hasSavedState = sessionStorage.getItem('ring-cum-bangle-scroll-position') || 
                         sessionStorage.getItem('ring-cum-bangle-last-product') || 
                         sessionStorage.getItem('ring-cum-bangle-visible-count') ||
                         sessionStorage.getItem('ring-cum-bangle-loaded-products');
  ...
  const handleProductClick = (product) => {
    ...
    sessionStorage.setItem('ring-cum-bangle-scroll-position', window.scrollY.toString());
    sessionStorage.setItem('ring-cum-bangle-last-product', product._id || product.id);
    sessionStorage.setItem('ring-cum-bangle-visible-count', visibleCount.toString());
    
    // Save the actual loaded products data to prevent reloading
    const loadedProducts = sortedProducts.slice(0, visibleCount);
    sessionStorage.setItem('ring-cum-bangle-loaded-products', JSON.stringify(loadedProducts));
    
    // Navigate to product page
    window.location.href = `/ring-cum-bangle/${product.slug || product._id || product.id}`;
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="relative text-white pt-16 sm:pt-32 pb-24 sm:pb-72 overflow-hidden">
        ...
        <div className="absolute inset-0">
          <Image
            src="/banner/ring-cum-bangle1.png"
