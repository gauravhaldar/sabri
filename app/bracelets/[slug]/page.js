...
export default function BraceletsProductPage({ params }) {
      ...
      <div className="min-h-screen bg-white">
        <div className="container mx-auto px-4 py-16 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/bracelets" className="text-blue-600 hover:underline">
              Back to Bracelets
            </Link>
          </div>
        </div>
  ...
  useEffect(() => {
    try {
      const key = "recentlyViewedBracelets";
                ...
                {expandedSections.description && (
                  <div className="mt-3">
                    <p className="text-sm text-gray-600 leading-relaxed">
                      {product.description ||
                        "Shine brighter this festive season with this elegant bracelet. Curated for those who love a perfect balance of glamour and grace, this item reflects brilliance in every detail."}
                    </p>
                  </div>
                )}
              </div>
          ...
          {activeProductsTab === "related" && relatedProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedProducts.map((rp) => (
                <Link key={rp.id} href={`/bracelets/${rp.id}`} className="group border border-neutral-200">
                  ...
                  <div className="px-2 py-2">
                    ...
                    <div className="mt-1 flex items-center gap-2 text-[13px]">
                      ...
                      {rp.originalPrice && (<span className="text-neutral-500 line-through">₹{rp.originalPrice.toLocaleString()}</span>)}
                    </div>
                  </div>
                </Link>
              ))}
          ...
          {activeProductsTab === "recent" && recentProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {recentProducts.map((rv) => (
                <Link key={rv.id} href={`/bracelets/${rv.id}`} className="group border border-neutral-200">
                  <div className="relative aspect-square overflow-hidden">
                  ...
                  <div className="px-2 py-2">
                    ...
                    <div className="mt-1 flex items-center gap-2 text-[13px]">
                      ...
                      {rv.originalPrice && (<span className="text-neutral-500 line-through">₹{rv.originalPrice.toLocaleString()}</span>)}
                    </div>
                  </div>
                </Link>
              ))}
