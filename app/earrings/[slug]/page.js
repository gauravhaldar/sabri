...
import { Heart, ShoppingBag, Star, ArrowLeft, Share2, Minus, Plus } from "lucide-react";
...
const productData = {
  e1: {
    ...
    reviews: 167,
    description:
      "Classic diamond stud earrings for everyday elegance. Perfect for any occasion.",
    features: [
      "VS1 Diamonds",
      "18K Gold Setting",
      "Hypoallergenic",
  ...
  e2: {
    ...
    reviews: 134,
    description:
      "Sophisticated pearl drop earrings for special occasions. Handcrafted with premium pearls.",
    features: [
      "Premium Pearls",
      "18K Gold Posts",
      "Handcrafted",
...
export default function ProductPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/earrings" className="text-blue-600 hover:underline">
            Back to Earrings
          </Link>
        </div>
      </div>
  ...
  useEffect(() => {
    try {
      const key = "recentlyViewedEarrings";
...
{/* 
              ...
              <div className="mt-6 bg-[#F1EEE4] p-4 rounded-lg">
                <div className="grid grid-cols-3 gap-3">
                  <div className="aspect-square bg-white rounded-lg shadow-sm flex flex-col items-center justify-center text-center">
          ...
          {activeProductsTab === "related" && relatedProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedProducts.map((rp) => (
                <Link key={rp.id} href={`/earrings/${rp.id}`} className="group border border-neutral-200">
                  <div className="relative aspect-square overflow-hidden">
                    <Image src={rp.image || rp.gallery?.[0]} alt={rp.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                  </div>
          ...
          {activeProductsTab === "recent" && recentProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {recentProducts.map((rv) => (
                <Link key={rv.id} href={`/earrings/${rv.id}`} className="group border border-neutral-200">
                  <div className="relative aspect-square overflow-hidden">
                    <Image src={rv.image || rv.gallery?.[0]} alt={rv.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
                  </div>
                  <div className="px-2 py-2">
