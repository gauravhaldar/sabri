...
const productData = {
  fg1: {
    ...
    reviews: 89,
    description:
      "Premium 18K gold wedding ring with classic design. Crafted with the finest materials.",
    features: ["18K Gold", "Handcrafted", "Lifetime Warranty", "Certified"],
  },
  fg2: {
    id: "fg2",
    ...
    reviews: 156,
    description:
      "Luxurious gold chain necklace for special occasions. Perfect for gifting or personal use.",
    features: [
      "18K Gold",
      "Adjustable Length",
      "Tarnish Resistant",
...
export default function ProductPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/fine-gold" className="text-blue-600 hover:underline">
            Back to Fine Gold
          </Link>
        </div>
      </div>
  ...
  useEffect(() => {
    try {
      const key = "recentlyViewedFineGold";
          ...
          {activeProductsTab === "related" && relatedProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedProducts.map((rp) => (
                <Link key={rp.id} href={`/fine-gold/${rp.id}`} className="group border border-neutral-200">
          ...
          {activeProductsTab === "recent" && recentProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {recentProducts.map((rv) => (
                <Link key={rv.id} href={`/fine-gold/${rv.id}`} className="group border border-neutral-200">
