...
const productData = {
  n1: {
    ...
    reviews: 124,
    description:
      "Elegant pearl drop necklace for special occasions. Handcrafted with premium pearls.",
    features: [
      "Premium Pearls",
      "18K Gold Chain",
      "Handcrafted",
  ...
  n2: {
    ...
    reviews: 98,
    description:
      "Classic gold chain necklace for everyday wear. Timeless design that never goes out of style.",
    features: [
      "18K Gold",
      "Adjustable Length",
      "Tarnish Resistant",
...
export default function ProductPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/necklaces" className="text-blue-600 hover:underline">
            Back to Necklaces
          </Link>
        </div>
      </div>
  ...
  useEffect(() => {
    try {
      const key = "recentlyViewedNecklaces";
          ...
          {activeProductsTab === "related" && relatedProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedProducts.map((rp) => (
                <Link key={rp.id} href={`/necklaces/${rp.id}`} className="group border border-neutral-200">
          ...
          {activeProductsTab === "recent" && recentProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {recentProducts.map((rv) => (
                <Link key={rv.id} href={`/necklaces/${rv.id}`} className="group border border-neutral-200">
