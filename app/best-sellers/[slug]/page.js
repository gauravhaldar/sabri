...
import { Heart, ShoppingBag, Star, ArrowLeft, Plus, Minus, Share2 } from "lucide-react";
...
const productData = {
  bs1: {
    ...
    reviews: 128,
    description:
      "Timeless classic gold wedding ring crafted with precision and elegance. Perfect for special occasions and everyday wear.",
    features: ["18K Gold", "Handcrafted", "Lifetime Warranty", "Free Shipping"],
  },
  bs2: {
    id: "bs2",
    ...
    reviews: 95,
    description:
      "Elegant diamond solitaire ring featuring a brilliant cut diamond in a classic setting.",
    features: [
      "VS1 Diamond",
      "18K Gold Setting",
      "Certified",
...
export default function ProductPage({ params }) {
  ...
  // Related products (simple: other items in dataset)
  const relatedProducts = Object.values(productData).filter((p) => p.id !== product.id).slice(0, 8);

  // Recently viewed (persist in localStorage by id)
  const [recentProducts, setRecentProducts] = useState([]);
  const [activeProductsTab, setActiveProductsTab] = useState("related");

  useEffect(() => {
    try {
      const key = "recentlyViewedBestSellers";
  ...
  const ProductMiniCard = ({ item }) => (
    <Link href={`/best-sellers/${item.id}`} className="group border border-neutral-200">
      <div className="relative aspect-square overflow-hidden">
        <Image src={item.image || item.gallery?.[0]} alt={item.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
      </div>
      <div className="px-2 py-2">
        <div className="line-clamp-1 text-sm text-neutral-900">{item.name}</div>
        <div className="mt-1 flex items-center gap-2 text-[13px]">
          <span className="font-medium text-neutral-900">₹{item.price.toLocaleString()}</span>
          {item.originalPrice && (
            <span className="text-neutral-500 line-through">₹{item.originalPrice.toLocaleString()}</span>
          )}
        </div>
      </div>
    </Link>
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8 pt-40">
        {/* Breadcrumb / Back */}
        <Link href="/best-sellers" className="inline-flex items-center text-gray-600 hover:text-gray-800 mb-4">
          <ArrowLeft className="w-4 h-4 mr-2" /> Back to Best Sellers
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-3">
            <div className="relative aspect-square overflow-hidden border border-neutral-200">
              <Image src={product.gallery[selectedImage]} alt={product.name} fill className="object-cover" />
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {product.gallery.map((image, index) => (
                ...
                >
                  <Image src={image} alt={`${product.name} ${index + 1}`} width={140} height={140} className="object-cover w-full h-full" />
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Details right */}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {product.isBestSeller && (
                  <span className="rounded bg-yellow-100 text-yellow-800 text-xs px-2 py-1">Best Seller</span>
                )}
              </div>
            </div>

            <div className="flex items-end gap-3">
              <span className="text-3xl font-semibold text-neutral-900">₹{product.price.toLocaleString()}</span>
              {product.originalPrice && (
                <>
                  <span className="text-lg text-neutral-500 line-through">₹{product.originalPrice.toLocaleString()}</span>
                  <span className="text-sm rounded bg-red-100 text-red-700 px-2 py-1">{product.discount}% OFF</span>
                </>
              )}
            </div>
              <p className="text-sm text-neutral-500">Inclusive of all taxes</p>
              <p className="text-sm text-neutral-500">SKU: DG-BUNDLE-3</p>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 p-4 rounded-md">
                ...
                {openDesc && (
                  <div className="mt-3 px-1 text-sm text-neutral-700 leading-relaxed">
            ...
            {activeProductsTab === "related" && relatedProducts.length > 0 && (
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {relatedProducts.map((rp) => (
                  <ProductMiniCard key={rp.id} item={rp} />
                ))}
              </div>
            )}
            {activeProductsTab === "recent" && recentProducts.length > 0 && (
              <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {recentProducts.map((rv) => (
                  <ProductMiniCard key={rv.id} item={rv} />
                ))}
              </div>
            )}
          </section>
