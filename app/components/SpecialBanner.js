  ...
  return (
    <section className="w-full bg-black">
      <div className="mx-auto max-w-[1920px]">
        {/* Title */}
        <div className="py-3 text-center text-xs tracking-wider text-white/80">
          {title}
        </div>

        {/* Banner */}
        <div className="relative h-[46vw] max-h-[560px] min-h-[260px] w-full">
          {slides.map((s, i) => (
            <div
              key={i}
            ...
            >
              ...
              {/* Right copy (optional) */}
              {s.caption && (
                <div className="pointer-events-none absolute right-[6%] top-1/2 -translate-y-1/2 text-right text-white">
                  <p className="text-3xl md:text-5xl font-semibold tracking-wider">{s.caption}</p>
                  {s.sub && <p className="mt-4 text-xl md:text-3xl tracking-widest">{s.sub}</p>}
                  {s.cta && (
                    <span className="mt-6 inline-block rounded border border-white/70 px-5 py-2 text-xs tracking-widest">
                      {s.cta}
                    </span>
                  )}
                </div>
          ...
          {/* Progress bar */}
          <div className="absolute inset-x-0 bottom-3 mx-[2%] h-[2px] bg-white/30">
          ...
          <button
            ...
            className="absolute bottom-2 right-3 grid h-8 w-8 place-items-center rounded-full bg-white/30 text-white backdrop-blur-sm"
          >
            {paused ? (
