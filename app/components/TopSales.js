  ...
  return (
    <section className="w-full bg-white">
      <div className="mx-auto max-w-7xl px-0 sm:px-1 lg:px-2">
        ...
        {/* Tabs */}
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 pb-6">
          {TABS.map((t) => (
            <button
              key={t.key}
          ...
          {filtered.map((p) => (
            <article key={p.id} className="group border border-neutral-200">
              <div className="relative aspect-[4/5] overflow-hidden">
                {p.badge && (
                  <span className="absolute left-3 top-3 z-10 rounded bg-white/90 px-2 py-1 text-[11px] font-medium tracking-wide">
                    {p.badge}
                  </span>
                )}
