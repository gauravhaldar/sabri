...
const productData = {
  c1: {
    ...
    reviews: 78,
    description:
      "Exclusive heritage collection ring with traditional design. A timeless piece that celebrates our rich cultural heritage.",
    features: [
      "Traditional Design",
      "18K Gold",
      "Handcrafted",
  ...
  c2: {
    ...
    reviews: 112,
    description:
      "Contemporary modern collection necklace. Sleek design that complements any outfit for any occasion.",
    features: [
      "Contemporary Design",
      "Premium Materials",
      "Versatile Style",
...
export default function ProductPage({ params }) {
  ...
  useEffect(() => {
    try {
      const key = "recentlyViewedCollections";
          ...
          {activeProductsTab === "related" && relatedProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relatedProducts.map((rp) => (
                <Link key={rp.id} href={`/collections/${rp.id}`} className="group border border-neutral-200">
          ...
          {activeProductsTab === "recent" && recentProducts.length > 0 && (
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {recentProducts.map((rv) => (
                <Link key={rv.id} href={`/collections/${rv.id}`} className="group border border-neutral-200">
