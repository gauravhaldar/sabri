...
export default function GiftsPage() {
  ...
  useEffect(() => {
    const savedPosition = sessionStorage.getItem('gifts-scroll-position');
    const savedProductId = sessionStorage.getItem('gifts-last-product');
    const savedVisibleCount = sessionStorage.getItem('gifts-visible-count');
    const savedProducts = sessionStorage.getItem('gifts-loaded-products');
    ...
    if (savedProducts) {
      sessionStorage.removeItem('gifts-loaded-products');
    }
  }, []);
        ...
        if (productIndex !== -1) {
          ...
          if (itemsNeeded > visibleCount) {
            ...
            setTimeout(() => {
              ...
              // Clear saved state
              sessionStorage.removeItem('gifts-last-product');
              sessionStorage.removeItem('gifts-scroll-position');
              sessionStorage.removeItem('gifts-visible-count');
  ...
  useEffect(() => {
    ...
    const hasSavedState = sessionStorage.getItem('gifts-scroll-position') || 
                         sessionStorage.getItem('gifts-last-product') || 
                         sessionStorage.getItem('gifts-visible-count') ||
                         sessionStorage.getItem('gifts-loaded-products');
  ...
  const handleProductClick = (product) => {
    ...
    sessionStorage.setItem('gifts-scroll-position', window.scrollY.toString());
    sessionStorage.setItem('gifts-last-product', product._id || product.id);
    sessionStorage.setItem('gifts-visible-count', visibleCount.toString());
    
    // Save the actual loaded products data to prevent reloading
    const loadedProducts = sortedProducts.slice(0, visibleCount);
    sessionStorage.setItem('gifts-loaded-products', JSON.stringify(loadedProducts));
    
    // Navigate to product page
    window.location.href = `/gifts/${product.slug || product._id || product.id}`;
  };
